...
function initSubGroups(map) {
    ...
    L.control.layers(null, null, { collapsed: true })
        .addOverlay(repairSectionSubGroup, 'Tous les ateliers'+getIconForControlLayer(repairIconUrl))
    ...
    WorkshopEnum.properties = {
        1: {icon: repairIcon, titlePrefix: 'Tous les ateliers', group: repairSectionSubGroup},
 ...
 */
 function getTypeRepair(fabrik, fabrik2, bicycle, bicycle2, camera, camera2, computers, computers2, fabrik, fabrik2, furniture, furniture2, small_electronics_device, small_electronics_device2) {
    var typeArray = [];


	if (fabrik=="yes" || fabrik2=="yes") {
		typeArray.push(WorkshopEnum.FABRIK);
	}

	if (furniture=="yes" || furniture2=="yes") {
		typeArray.push(WorkshopEnum.FURNITURE);
	}

	if (computers=="yes" || computers2=="yes") {
		typeArray.push(WorkshopEnum.COMPUTERS);
	}

	if (camera=="yes" || camera2=="yes") {
		typeArray.push(WorkshopEnum.CAMERA);
	}

	if (bicycle=="yes" || bicycle2=="yes") {
		typeArray.push(WorkshopEnum.BICYCLE);
	}

	if (small_electronics_device=="yes" || small_electronics_device2=="yes") {
		typeArray.push(WorkshopEnum.SMALLELECTRONICSDEVICES);
	}

    // On met toujours "ALL_REPAIR"
    typeArray.push(WorkshopEnum.ALL_REPAIR);
 ...
 */
function getHtmlFormattedAddress(housename, housename2, housename3, housenumber, housenumber2, housenumber3, street, street2, street3, postcode, postcode2, postcode3, city, city2, city3) {
    var address = "";

    if (street && housenumber) {
        address += housenumber+' '+street+'<br />';
    } else if (street2 && housenumber2) {
        address += housenumber2+' '+street2+'<br />';
    } else if (street3 && housenumber3) {
        address += housenumber3+' '+street3+'<br />';
    } else if (street) {
        address += street+'<br />';
    } else if (street2) {
        address += street2+'<br />';
    ...
    } else if (city2 && postcode) {
        address += postcode2+' '+city2+'<br />';
    } else if (city3 && postcode3) {
        address += postcode3+' '+city3+'<br />';
    } else if (city) {
        address += city+'<br />';
    } else if (city2) {
        address += city2+'<br />';
